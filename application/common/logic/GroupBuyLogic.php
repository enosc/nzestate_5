...
class GroupBuyLogic extends Promotion
{
    ...
    public function __construct($goods,$specGoodsPrice)
    {
        ...
        if($this->specGoodsPrice){
            //活动商品有规格，规格和活动是一对一
            $this->GroupBuy = GroupBuy::get($specGoodsPrice['promotion_id']);
        }else{
            //活动商品没有规格，活动和商品是一对一
            $this->GroupBuy = GroupBuy::get($this->goods['promotion_id']);
        }
        if ($this->GroupBuy) {
            ...
            if ($this->checkActivityIsEnd() && $this->GroupBuy['is_end'] == 0) {
                if($this->specGoodsPrice){
                    Db::name('goods_spec_price')
                        ->where('item_id', $this->specGoodsPrice['item_id'])
                        ->update(['promotion_type' => 0, 'promotion_id' => 0]);
                    $goodsPromCount = Db::name('spec_goods_price')
                        ->where('goods_id', $this->specGoodsPrice['goods_id'])
                    ...
                    $this->specGoodsPrice = GoodsSpecPrice::get($specGoodsPrice['item_id']);
                }else{
                    Db::name('goods')
                        ->where("goods_id", $this->GroupBuy['goods_id'])
                        ->update(['promotion_type' => 0, 'promotion_id' => 0]);
                }
    ...
    public function buyNow($buyGoods){
        ...
        if($this->GroupBuy['is_end'] == 1 || empty($this->GroupBuy)){
            throw new NzestateException('团购商品立即购买',0,['status' => 0, 'msg' => '团购活动已结束', 'result' => '']);
        }
        if($this->checkActivityIsAble()){
            ...
            if($buyGoods['goods_num'] > $groupBuyPurchase){
                throw new NzestateException('团购商品立即购买',0,['status' => 0, 'msg' => '商品库存不足，剩余'.$groupBuyPurchase, 'result' => '']);
            }
            $buyGoods['user_goods_price'] = $this->GroupBuy['price'];
            $buyGoods['promotion_type'] = 2;
            $buyGoods['promotion_id'] = $this->GroupBuy['id'];
        }
        return $buyGoods;
    }
}