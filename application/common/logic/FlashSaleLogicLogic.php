...
class FlashSale extends Promotion
{
    ...
    public function __construct($goods, $specGoodsPrice)
    {
        parent::__construct();
        $this->goods = $goods;
        $this->goodsSpecPrice = $specGoodsPrice;
        $flash_sale_m = new \app\common\model\FlashSale();
        if($this->goodsSpecPrice){
            //活动商品有规格，规格和活动是一对一
            $this->flashSale = $flash_sale_m->get($specGoodsPrice['promotion_id']);
        }else{
            //活动商品没有规格，活动和商品是一对一
            $this->flashSale = $flash_sale_m->get($goods['promotion_id']);
        }
        if ($this->flashSale) {
            ...
            if ($this->checkActivityIsEnd() && $this->flashSale['is_end'] == 0) {
                if($this->goodsSpecPrice){
                    ...
                    $this->goodsSpecPrice = Model('GoodsSpecPrice')->get($specGoodsPrice['item_id']);
                }else{
                    Db::name('goods')
                        ->where("goods_id", $this->flashSale['goods_id'])
                        ->update(['promotion_type' => 0, 'promotion_id' => 0]);
                }
                $this->flashSale->is_end = 1;
                $this->flashSale->save();
                unset($this->goods);
                $this->goods = Model('Goods')->get($goods['goods_id']);
            }
        }
    }
    ...
    public function getUserFlashOrderGoodsNum($user_id){
        ...
        $order_id_arr = Db::name('order')
            ...
            ->where($orderWhere);
        if ($order_id_arr) {
            $orderGoodsWhere = ['promotion_id' => $this->flashSale['id'], 'promotion_type' => 1, 'order_id' => ['in', implode(',', $order_id_arr)]];
    ...
    public function buyNow($buyGoods){
        if($this->checkActivityIsAble()){
            if($buyGoods['goods_num'] > $this->flashSale['buy_limit']){
                throw new NzestateException('抢购商品立即购买', 0, ['status' => 0, 'msg' => '每人限购'.$this->flashSale['buy_limit'].'件', 'result' => '']);
            }
        }
        ...
        if($userBuyGoodsNum > $this->flashSale['buy_limit']){
            throw new NzestateException('抢购商品立即购买', 0, ['status' => 0, 'msg' => '每人限购'.$this->flashSale['buy_limit'].'件，您已下单'.$userFlashOrderGoodsNum.'件', 'result' => '']);
        }
        $flashSalePurchase = $this->flashSale['goods_num'] - $this->flashSale['buy_num'];//抢购剩余库存
        if($buyGoods['goods_num'] > $flashSalePurchase){
            throw new NzestateException('抢购商品立即购买', 0, ['status' => 0, 'msg' => '商品库存不足，剩余'.$flashSalePurchase, 'result' => '']);
        }
        $buyGoods['user_goods_price'] = $this->flashSale['price'];
        $buyGoods['promotion_type'] = 1;
        $buyGoods['promotion_id'] = $this->flashSale['id'];
        return $buyGoods;
    }
